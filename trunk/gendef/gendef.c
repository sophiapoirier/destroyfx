...
static void decode_mangle (const char *n);
...
opt_chain (const char *opts, const char *next)
{
  ...
  current = (Gendefopts*)malloc (sizeof(Gendefopts));
  if (current)
    {
...
load_pep (void)
{
  ...
  if (gPEDta->FileHeader.SizeOfOptionalHeader == IMAGE_SIZEOF_NT_OPTIONAL32_HEADER
    ...
    && gPEPDta->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
    {
      ...
      {
        IMAGE_OPTIONAL_HEADER64 *hdr = &gPEPDta->OptionalHeader;
        printf("Magic\t   %" PRIu16 "\n", hdr->Magic);
        ...
        printf("ImageBase\t     %" PRIu64 "\n", hdr->ImageBase);
        printf("SectionAlignment\t      %" PRIu32 "\n", hdr->SectionAlignment);
        printf("FileAlignment\t %" PRIu32 "\n", hdr->FileAlignment);
        printf("MajorOperatingSystemVersion\t   %" PRIu16 "\n", hdr->MajorOperatingSystemVersion);
        printf("MinorOperatingSystemVersion\t   %" PRIu16 "\n", hdr->MinorOperatingSystemVersion);
        printf("MajorImageVersion\t     %" PRIu16 "\n", hdr->MajorImageVersion);
        printf("MinorImageVersion\t     %" PRIu16 "\n", hdr->MinorImageVersion);
        printf("MajorSubsystemVersion\t %" PRIu16 "\n", hdr->MajorSubsystemVersion);
        printf("MinorSubsystemVersion\t %" PRIu16 "\n", hdr->MinorSubsystemVersion);
        ...
        printf("DllCharacteristics\t    %" PRIu16 "\n", hdr->DllCharacteristics);
  ...
  while ((exp = gExp) != NULL)
    {
      ...
      if (exp->name[0] == 0)
        printf( "ord_%u", (unsigned int) exp->ord);
      else
        {
          const char *quote = strchr (exp->name, '.') ? "\"" : "";
          printf( "%s%s%s", quote, exp->name, quote);
        }
      ...
      if (exp->retpop != (uint32_t) -1 && !exp->be64 && has_atdecoration())
        {
          if (exp->name[0]=='?')
            printf(" ; has WINAPI (@%u)", (unsigned int) exp->retpop);
          else
            printf("@%u", (unsigned int) exp->retpop);
        }
      if (exp->func == 0 && no_forward_output == 0)
        printf( " = %s", exp->forward);
      if (exp->name[0] == 0)
        printf(" @%u", (unsigned int) exp->ord);
      if (exp->beData)
        printf(" DATA");
      ...
      if (exp->retpop != (uint32_t) -1 || (exp->retpop == 0 && exp->be64) || !has_atdecoration ())
        ...
        {
          printf( " ; Check!!! forwards to %s in %s (ordinal %u)",
            pimpname->name, pimpname->dll, pimpname->ord);
        }
      else if (exp->func == 0 && !exp->beData)
        {
          printf( " ; Check!!! forwards to %s", exp->forward);
        }
      else if (seen_ret == 0 && !exp->beData)
        {
          printf( " ; Check!!! Couldn't determine function argument count. Function doesn't return. ");
        }
      printf("\n");
      free (exp);
    }
