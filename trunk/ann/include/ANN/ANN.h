...
#ifdef WIN32
/* XXX for Destroy FX, since we don't build ANN as a DLL but instead
   include it within our own DLLs, perhaps we should turn off ANN exports? */
  //----------------------------------------------------------------------
...
#define ANNversionCmt	"(destroyfx minifork)"
...
/* For Destroy FX, we rarely need double precision, since
   samples are only single precision in the first place. 
   Plus DFX is lo-fi.
*/
...
//		files, some suggested values are listed below, assuming 64-bit
...
//	strictly positive.
//----------------------------------------------------------------------
/* false here might make sense for Destroy FX, depending on the effect... */
...
//			metric.
//
//		Points (somewhat like strings in C) are stored as
...
//				Allocate and deallocate storage for a single point, and
