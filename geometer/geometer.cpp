...
void PLUGIN::setParameter(long index, float value) {
   ...
   /* copy the new value to the active program's corresponding 
      parameter value */
   if ( (index >= 0) && (index < NUM_PARAMS) )
     programs[curProgram].param[index] = value;
}

/* save & restore parameter data using chunks so that we can save 
   MIDI CC assignments */
...
void PLUGIN::processX(float **trueinputs, float **trueoutputs, long samples,
   ...
   for (int ii = 0; ii < samples; ii++) {
     ...
     if (insize == framesize) {
       ...
       memcpy(prevmix, out0 + outstart + outsize + third, third * 
              sizeof (float));
